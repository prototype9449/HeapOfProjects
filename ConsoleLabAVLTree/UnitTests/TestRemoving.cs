...
namespace UnitTests
{
    ...
    public class TestRemoving
    {
        ...
        [TestMethod]
        public void AfterRemoveAtRightLeafNodeTraversalMustMatchIfElementsFive()
        {
            var tree = GetBinaryTree(new[] { 100, 80, 120, 60, 90 });
            tree.Remove(120);
            var matchNumbers = new int[] { 80, 60, 100, 90};
            int index = 0;
            foreach (var node in tree)
            {
                Assert.AreEqual(matchNumbers[index], node.Key);
        ...
        [TestMethod]
        public void AfterRemoveAtLeftLeafNodeTraversalMustMatchIfElementsFive()
        {
            var tree = GetBinaryTree(new[] { 100,120,90,110,130});
            tree.Remove(90);
            var matchNumbers = new int[] {120,100,130,110 };
            int index = 0;
            foreach (var node in tree)
            {
                Assert.AreEqual(matchNumbers[index], node.Key);
